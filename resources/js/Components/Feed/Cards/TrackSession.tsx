...
export default function TrackSession({ id, subject, parent, user, description, updated_at }: any) {
    ...
    return (
        <FeedCard>
            ...
            <CardTitle>
                ...
                >
                    {parent.track_layout.track.name} {(parent.track_layout.name) ? `- ${parent.track_layout.name}` : ''}
                </Link>
            </CardTitle>
            <CardSubtitle>
            ...
            {parent.sessions.length !== 0 && <SessionAccordion visit={parent} profile />}
            <FeedFooter id={id} />
        </FeedCard>
    )
}