...
export default function SessionLaps({ id, subject, user, description, updated_at }: any) {
                ...
                <div className={(reveal) ? 'relative' : 'h-24 overflow-hidden relative'}>
                    <div className="flex flex-col sm:flex-row items-center divide-y sm:divide-y-0 sm:divide-x border-t text-xs font-medium text-gray-500 dark:text-gray-500 dark:border-gray-700 dark:divide-gray-700">
                        <div className="px-5 py-0.5">
                            Session Length: {subject.session_length} {subject.session_length_type}
                        </div>
                        <div className="px-5 py-0.5">
                        ...
                        </div>
                        {
                            subject.fastestLap && (
                                <div className="px-5 py-0.5">
                                    Fastest Lap: {FormatLapTime(subject.fastestLap)} on lap {subject.fastestLap.lap_number}
                                </div>
                            )
                        }
                        {
                            subject.finish_position && subject.total_drivers && (
                                <div className="px-5 py-0.5">
                                    Finished {toOrdinal(subject.finish_position)} of {subject.total_drivers} drivers
                                </div>
                            )
                        }
                    </div>
                    <SessionLapsTable session={subject} />
                    <div className={(reveal) ? 'border-t dark:border-gray-700 pt-1' : 'absolute bottom-0 inset-x-0'}>
                        <button
