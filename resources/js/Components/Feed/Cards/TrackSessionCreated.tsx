  ...
  return (
    <FeedCard>
      <FeedHeader
        ...
        description={
          <>
            added {parent.sessions.length} Track{' '}
            {parent.sessions.length === 1 ? 'Session' : 'Sessions'}
          </>
        }
      ...
      <div className="border dark:border-gray-700 rounded-md p-2">
        <CardTitle>
          {parent.title} at {parent.track_layout.track.name}{' '}
        ...
        <div className="mt-2 text-xs text-gray-800 dark:text-gray-400">
          {parent.sessions.length > 0 && (
            <span>
              ...
              {parent.sessions.length === 1 ? 'Session' : 'Sessions'} covering{' '}
              {laps} laps.{' '}
            </span>
          )}
          {parent.fastestLap && (
            <span>Fastest Lap: {FormatLapTime(parent.fastestLap)}</span>
          )}
        </div>
      </div>
