...
export default function AppSearchResults({ results, clearResults }: any) {
    ...
    const tabClass = (i: number) => classNames(
        'border-b-2',
    ...
    return (
        <div className="flex flex-col divide-y dark:divide-gray-600">
            <div className="flex flex-row items-center gap-x-4 px-3">
                ...
                <div className="text-xs ml-auto cursor-pointer hover:text-brand-600 mb-2"
            ...
            {
                selected === 0 && results?.all && results.all.map((result: any, i: number) => {
                    if (result.email) {
            ...
            {
                selected === 1 && results?.tracks && results.tracks.map((result: any, i: number) => {
                    return <ResultItemTrack result={result} key={i} />
                })
            }
            {
                selected === 2 && results?.people && results.people.map((result: any, i: number) => {
                    return <ResultItemPerson result={result} key={i} />
                })
            }
        </div>
