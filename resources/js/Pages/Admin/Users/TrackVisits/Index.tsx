                    ...
                    <ul role="list" className="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700 border-x border-gray-200 dark:border-gray-700 rounded-t-md">
                        <li>
                            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 items-center gap-x-6 p-2 bg-gray-200 dark:bg-gray-700 rounded-t-md">
                                <p className="text-xs font-medium text-left text-gray-500 dark:text-gray-400 uppercase">Visit</p>
                                <p className="hidden sm:block text-xs font-medium text-left text-gray-500 dark:text-gray-400 uppercase">Track</p>
                            ...
                            >
                                <div className="flex min-w-0 gap-x-4">
                                    <div className="min-w-0 flex-auto">
                                        <p className="text-base font-semibold leading-6 text-gray-900 dark:text-white">{visit.title}</p>
                                        <p className="truncate text-sm leading-5 text-gray-500 dark:text-gray-400">
                                            {moment(visit.visit_date).format('Do MMMM YYYY [at] HH:mm')}
                                        </p>
                                    ...
                                    <p>
                                        {
                                            visit.sessions.map((session: any, i: number) => (
                                                <span key={i} className="after:content-['/'] last:after:content-['']">{session.laps.length}</span>
                                            ))
                                        }
                                    </p>
                                </div>
