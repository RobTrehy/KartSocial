...
export default function AdminAppeals({ ban }: any) {
    ...
    const cancelAppeals = (e: MouseEvent<HTMLAnchorElement>) => {
        e.preventDefault();
        axios.post(route('admin:user.restriction.appeal.close', { ban: ban.id }))
            .then(res => {
                router.reload();
            });
    }
    ...
    const openAppeals = (e: MouseEvent<HTMLAnchorElement>) => {
        e.preventDefault();
        axios.put(route('admin:user.restriction.appeal.open', { ban: ban.id }))
            .then(res => {
                router.reload();
            });
    }
    ...
    return (
        <div className="last:rounded-b-lg border dark:border-gray-700 w-full overflow-hidden divide-y dark:divide-gray-700">
            ...
            <div className="bg-white dark:bg-gray-900 text-gray-600 dark:text-gray-300 flex flex-row justify-around py-2 px-3 w-full">
                ...
                <a href="#" className="text-brand-500" onClick={(e: MouseEvent<HTMLAnchorElement>) => { e.preventDefault(); setChangingExpiry(true) }}>
                ...
                <DialogModal.Content title="Change Restriction Expiry Date">
                    <div className="flex flex-wrap items-center gap-x-2 mb-2">
                        ...
                        <Select
                            name="from"
                            value={from}
                            onChange={(e: ChangeEvent<HTMLInputElement>) => { setFrom(e.currentTarget.value) }}
                        >
                        ...
                        <Select
                            name="method"
                            value={method}
                            onChange={(e: ChangeEvent<HTMLInputElement>) => { setMethod(e.currentTarget.value) }}
                        >
                        ...
                        <Select
                            name="count"
                            value={count}
                            onChange={(e: ChangeEvent<HTMLInputElement>) => { setCount(+(e.currentTarget.value)) }}
                        >
                            <option value="0">0</option>
                            <option value="1">1</option>
                            <option value="2">2</option>
                            <option value="3">3</option>
                            {(type === 'days' || type === 'months') && <option value="4">4</option>}
                            {(type === 'days' || type === 'months') && <option value="5">5</option>}
                            {(type === 'days' || type === 'months') && <option value="7">6</option>}
                        ...
                        <Select
                            name="type"
                            value={type}
                            onChange={(e: ChangeEvent<HTMLInputElement>) => { setType(e.currentTarget.value) }}
                        >
                    ...
                    <p><span className="font-semibold">New Expiry:</span> {moment(preview).format('Do MMMM YYYY')}</p>
                    <InputError className="mt-2" message={expiryForm.errors.expires_at} />
                </DialogModal.Content>

                <DialogModal.Footer>
                    <SecondaryButton onClick={() => setChangingExpiry(false)}>Cancel</SecondaryButton>

                    <PrimaryButton
                        className={classNames('ml-2', { 'opacity-25': expiryForm.processing })}
