    ...
    const processImport = () => {
        importLaps.map((lap, i) => {
            if (isNaN(lap.lap_time)) {
                importLaps[i].lap_time = lap.lap_time.split(':').reduce((acc: number, time: number) => (60 * acc) + +time).toString();
            }
            if (lap.lap_number !== "1") {
                let prevLap = (importLaps[i - 1]) ? importLaps[i - 1] : laps[laps.length - 1];
                if (prevLap) {
                ...
                <div className="py-12">
                    <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                        <div className="overflow-hidden sm:rounded-lg">
                            <CreateSessionLapForm session={session} laps={laps} setLaps={setLaps} />
                        </div>
                    </div>
                </div>
