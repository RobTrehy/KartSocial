...
import React, { ChangeEvent, FocusEvent, KeyboardEvent, MouseEvent, useEffect, useState } from 'react';

export default function CreateSessionLapForm(props: any) {
  const route = useRoute();

  const [timeFocus, setTimeFocus] = useState<number | null>(null);
  const [deleteBin, setDeleteBin] = useState<Array<any>>([]);
  ...
  const createSessionLap = () => {
    if (deleteBin.length) {
      deleteBin.map((lap: any, i: number) => {
        axios.delete(route('session.lap.destroy', { 'session': props.session.id, 'lap': lap.id }));
      })
    }
  ...
  const addNewLap = () => {
    props.setLaps([...props.laps, {
      lap_number: (props.laps.length) ? +(props.laps[props.laps.length - 1].lap_number) + 1 : "1",
      lap_time: '0',
      lap_diff: '',
    }]);
  }
  ...
  const changeLapTimeValue = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (props.laps[i].lap_number != '1' && i !== '0') {
      props.laps[i].lap_diff = (props.laps[i].lap_time - props.laps[+(i) - 1].lap_time).toFixed(3);
    }
  ...
  const maskedLapTime = (i: number) => {
    let time = props.laps[i].lap_time;
    if (time === '0') {
      return '00:00.000';
    } else {
  ...
  const lapTimeFocus = (e: FocusEvent<HTMLInputElement> | MouseEvent<HTMLInputElement>) => {
  ...
  const lapTimeBlur = (i: number) => {
    props.laps[i].lap_time = props.laps[i].lap_time.split(':').reduce((acc: number, time: number) => (60 * acc) + +time)
    props.setLaps([...props.laps]);
    setTimeFocus(null);
  }
  ...
  const moveLapUp = (i: number) => {
    ...
    // Reset diffs
    props.laps.map((lap: any, i: number) => {
  ...
  return (
    ...
    >
      <div className="flex bg-gray-100 dark:bg-gray-900 dark:text-white text-xs mb-1.5 rounded-md">
        ...
        <div className="flex-1 px-3">Time</div>
          ...
          return (
            <div className="flex mb-1.5 items-center" key={i}>
              <div className="border border-gray-300 dark:border-gray-800 shadow-sm rounded-l-md py-2 px-3 w-24 bg-gray-100 dark:bg-gray-600 flex justify-between items-center">
                ...
                <button
                  type="button"
                  className={`py-0.5 ${(i === 0) ? 'text-gray-800 cursor-not-allowed' : 'hover:text-blue-500'}`}
                  onClick={() => moveLapUp(i)}
                  title="Move Lap Up"
                >
                ...
                <button
                  type="button"
                  className={`py-0.5 ${(i === props.laps.length - 1) ? 'text-gray-800 cursor-not-allowed' : 'hover:text-green-500'}`}
              ...
              />
              {
                lap.lap_number != 1 ?
                  <div
                    className={`border border-gray-300 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-300 shadow-sm py-2 px-3 w-32 ${(lap.lap_diff < 0) ? 'bg-green-100 dark:bg-green-900' : 'bg-red-100 dark:bg-red-900'}`}
                  >
                    {FormatLapDiff(lap)}
                  </div>
                  :
                  <div className="border border-gray-300 dark:border-gray-800 shadow-sm py-2 px-3 w-32 bg-gray-100 dark:bg-gray-600">&nbsp;</div>
              }
