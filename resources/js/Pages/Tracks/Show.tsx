        ...
        <AppLayout title={track.name}>
            <div className="w-full aspect-[42/9] overflow-hidden bg-brand-700 relative mb-6 shadow-md">
                {
                    (track.lat && track.lng) && (
                        <div className="absolute inset-0">
                            <Map
                                defaultCenter={[+(track.lat), +(track.lng)]}
                                defaultZoom={13}
                            >
                                <Marker width={50} anchor={[+(track.lat), +(track.lng)]} hover={false} />
                            </Map>
                        </div>
                    )
                }
                ...
                <div className="relative flex justify-between items-end h-full max-w-7xl mx-auto sm:px-6 lg:px-8 pb-6">
                    <div className="flex flex-row gap-x-6 text-gray-800 dark:text-gray-200 items-end">
                        <div className="text-gray-100 flex flex-col gap-y-2">
                            ...
                            {
                                (track.address_1 || track.town || track.county || track.postal_code) && (
                                    <p className="text-gray-300 max-w-md">
                                        {track.address_1}{(track.address_2) ? ', ' : ''}
                                        {track.address_2}{(track.address_3) ? ', ' : ''}
                                        {track.address_3}
                                        {(track.town) ? ', ' : ''}{track.town}
                                        {(track.county) ? ', ' : ''}{track.county}
                                        {(track.postal_code) ? ', ' : ''}{track.postal_code}
                                    </p>
                                )
                            }
                            <div className="flex flex-row gap-x-2 text-md">
                                {
                                    track.url && <a href={track.url} target="_blank" rel="nofollow" className="hover:text-brand-500">{track.url}</a>
                                }
                                {
                                    (track.url && track.number) && <span className="text-gray-400">&bull;</span>
                                }
                                {
                                    track.number && <a href={`tel:${track.number}`} className="hover:text-brand-500" target="_blank" rel="nofollow">{track.number}</a>
                                }
                            </div>
                        </div>
                    </div>
                    <div className="mt-4 w-1/3 text-right text-gray-800 dark:text-gray-200">
                        {
                            (permissions.includes('tracks.layouts.create') || permissions.includes('tracks.layouts.update')) &&
                            <div className="inline-flex items-center divide-x divide-gray-300 dark:divide-gray-500 overflow-hidden backdrop-blur-sd bg-white/50 dark:bg-gray-800/50 border border-gray-300 dark:border-gray-500 rounded-md font-semibold text-xs text-gray-100 dark:text-gray-300 uppercase tracking-widest shadow-sm disabled:opacity-25">
                                {
                                    ...
                                    <span
                                        ...
                                        onClick={() => router.visit(route('tracks.layout.create', { 'track': track.id }))}
                                    >
                                        Add Layout
                                    </span>
                                }
                                {
                                    ...
                                    <span
                                        className="px-4 py-2 cursor-pointer hover:bg-gray-50/20 dark:hover:bg-gray-700/50 transition ease-in-out duration-150"
                                        onClick={() => router.visit(route('tracks.edit', { 'track': track.id }))}
                                    >
                                        Update Details
                                    </span>
                                }
            ...
            <div className="pb-8">
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    {
                        track.fastestLap && (track.fastestLap?.lap_time === track.myFastest?.lap_time) && (
                            <div className="text-center pb-8 lg:px-4">
                                <div className="p-2 bg-brand-800 items-center text-brand-100 leading-none lg:rounded-full flex lg:inline-flex" role="alert">
                                    ...
                                    <span className="font-semibold mr-2 text-left flex-auto">You hold the track record on Kart Social!</span>
                                </div>
                            </div>
                        )
                    }

                    {
                        layouts.length > 1 && (
                            <div className="flex flex-row justify-between items-center mb-4">
                                <button type="button" className="flex w-12" onClick={() => setLayout((layout === 0) ? layouts.length - 1 : layout - 1)}>
                                    <svg
                                ...
                                <div className="flex flex-col items-center">
                                    <p className="text-xl font-semibold">{layouts[layout].name} Layout</p>
                                    <p className="text-gray-500 text-xs">
                                        {((!layouts[layout].retired_at && !layouts[layout].is_default) ? "Alternate Layout " : '')}
                                        {((layouts[layout].retired_at) ? <span className='text-red-500'>Retired Layout </span> : '')}
                                        ({layout + 1}/{layouts.length})
                                    </p>
                                </div>
                                <button type="button" className="flex w-12" onClick={() => setLayout((layout === layouts.length - 1) ? 0 : layout + 1)}>
                                    <svg
                                        xmlns="http://www.w3.org/2000/svg"
                                        fill="none"
                                        viewBox="0 0 24 24"
