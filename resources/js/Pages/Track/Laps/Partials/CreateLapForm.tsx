...
import { TrackEvent, TrackSession, TrackSessionLap } from '@/types';
...
export default function CreateLapForm(props: Props) {
  const route = useRoute();

  const { session, laps } = props;

  const [timeFocus, setTimeFocus] = useState<number | null>(null);
  const [deleteBin, setDeleteBin] = useState<Array<TrackSessionLap>>([]);
  ...
  useEffect(() => {
    form.setData('laps', laps);
  }, [laps]);

  const createSessionLap = () => {
    if (deleteBin.length) {
      deleteBin.map((lap: TrackSessionLap, i: number) => {
        axios.delete(
          route('events.laps.destroy', {
  ...
  const addNewLap = () => {
    props.setLaps([
      ...laps,
      {
        lap_number: laps.length
          ? +laps[laps.length - 1].lap_number + 1
          : '1',
        lap_time: '0',
        lap_diff: '',
      },
  ...
  const changeLapTimeValue = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (e.target.value.length === 10) {
      ...
      mins = '0' + v[0][0];
      secs = v[0][1] + v[1][0];
      millis = v[1][1] + v[2][0] + v[2][1];
    }

    laps[i].lap_time = +mins * 60 + +secs + '.' + millis;

    if (laps[i].lap_number != '1' && i !== '0') {
      laps[i].lap_diff = (
        laps[i].lap_time - laps[+i - 1].lap_time
      ).toFixed(3);
    }

    calculateDiffs();
    props.setLaps([...laps], session.id);
  };

  const maskedLapTime = (i: number) => {
    let time: string = laps[i].lap_time.toString();
    if (time === '0') {
      return '00:00.000';
    } else {
  ...
  const lapTimeBlur = (i: number) => {
    laps[i].lap_time = laps[i].lap_time
      .split(':')
  ...
  const moveLapUp = (i: number) => {
    ...
    let lap = laps[i];
    lap.lap_number = i;
    laps[i - 1].lap_number = i + 1;

    laps.splice(i, 1);
    laps.splice(i - 1, 0, lap);
  ...
  const deleteLap = (i: number) => {
    ...
    // Reset lap numbers
    laps.map((lap: TrackSessionLap, i: number) => {
      laps[i].lap_number = i + 1;
    });
  ...
  return (
    <FormSection
      ...
      title={`${session.name} Laps`}
      description={`Manage your laps on this session.`}
      noGrid
      renderActions={() => (
        <>
        ...
        return (
          <div className="flex mb-1.5 items-center" key={i}>
            <div className="border border-gray-300 dark:border-gray-800 shadow-sm rounded-l-md py-2 px-3 w-24 bg-gray-100 dark:bg-gray-600 flex justify-between items-center">
              ...
              <button
                type="button"
                className={`py-0.5 ${i === laps.length - 1
                  ? 'text-gray-800 cursor-not-allowed'
