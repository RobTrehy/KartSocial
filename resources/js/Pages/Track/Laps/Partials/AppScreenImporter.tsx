...
export default function AppScreenImporter({ setProcessing }: any) {
  ...
  const processImage = async (name: string) => {
    if (!ocrDone.includes(name)) {
      ...
      const {
        ...
        URL.createObjectURL(files.filter(f => f.name === name)[0]),
      );
  ...
  const findLapsInText = () => {
    Object.keys(ocrData).map(i => {
      ...
      if (!data.processed && !data.processing) {
        ...
        let lapText =
          data.text.indexOf('lap #') < 0 ? 0 : data.text.indexOf('lap #');
        let avgText =
          data.text.indexOf('Avg.') < 0
            ? data.text.length
            : data.text.indexOf('Avg.');
        let lapLines = data.text
          .substr(lapText, avgText - lapText)
          .replace(/\r/g, '')
          .split(/\n/);
  ...
  return (
    <div className="flex flex-col">
      <span className="flex justify-center items-center text-sm mb-1 text-red-500">
        {message}
      </span>
          ...
          return (
            <div key={key} className="overflow-hidden relative">
              <i
                ...
                className="absolute bottom-0 right-0 bg-white opacity-50 hover:opacity-100 text-red-500 cursor-pointer"
              >
                <svg
