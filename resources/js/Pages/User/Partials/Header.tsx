...
export default function Header({ user, following }: Props) {
    ...
    return (
        <div className="w-full aspect-[42/9] overflow-hidden bg-center bg-cover relative mb-6 shadow-md"
            style={{
            ...
            <div className="relative flex justify-between items-end h-full max-w-7xl mx-auto sm:px-6 lg:px-8 pb-6">
                <div className="flex flex-row gap-x-6 text-gray-800 dark:text-gray-200 items-end">
                    <div className="h-52 w-52 p-1 rounded-md backdrop-blur-sm bg-white/30">
                        <img
                    ...
                    <div className="text-gray-100 flex flex-col gap-y-2">
                        ...
                        <div className="flex flex-row gap-x-2 text-md">
                            {
                                user.home_track && <Link href={route('tracks.show', { track: user.home_track.id })} className="hover:text-brand-500">{user.home_track.name}</Link>
                            }
                            {
                                (user.home_track && user.weight) && <span className="text-gray-400">&bull;</span>
                            }
                            {
                                ...
                                <>
                                    {
                                        (page.props.auth.user?.id !== user.id && !following) && (
                                            <span
                                                ...
                                                onClick={() => router.put(route('user.follow', { user: user.id }))}
                                            >
                                                Follow
                                            </span>
                                        )
                                    }
                                    {
                                        (page.props.auth.user?.id !== user.id && following) && (
                                            <span
                                                ...
                                                onClick={() => router.delete(route('user.unfollow', { user: user.id }))}
                                            >
                                                Unfollow
                                            </span>
                                        )
